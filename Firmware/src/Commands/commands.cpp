...
void Commands::SwitchCamPower(System& sm, const RnpPacketSerialized& packet)
{	
	SimpleCommandPacket commandpacket(packet);

	if(commandpacket.arg != 0 || commandpacket.arg != 1){ //Only allow arguments 0 and 1.
		return; 
	}
	
	digitalWrite(PinMap::BuckEN, commandpacket.arg); //0 turns the camera power off, 1 turns it on
}
